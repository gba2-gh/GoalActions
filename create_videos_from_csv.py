...
def extract_from_df(df, output_folder):
    for index,play in df.iterrows():
        if index > 99:
            break
        input_path = play['video_path']+ extraction_resolution+ exrtaction_format
        ...
        extract_video_segment( input_path, play['play_start'], play['play_end'], output_path)
        output_path = f"{output_folder}\\action_{play['id']}\\clip_MT.mp4"
        extract_video_segment(input_path, play['play_start_MT'], play['play_end_MT'], output_path)
    return
